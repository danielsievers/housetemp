...
def compare_timeseries_stats(golden_arr, current_arr, name, tolerance_rmse=0.5):
    """Compare two timeseries using RMSE instead of exact match."""
    g = np.array(golden_arr)
    c = np.array(current_arr)
    
    if len(g) != len(c):
        return f"{name}: Length mismatch ({len(g)} vs {len(c)})"
    
    rmse = np.sqrt(np.mean((g - c)**2))
    
    # Also useful: Mean Error
    mean_diff = np.mean(c - g)
    
    print(f"    [STATS] {name}: RMSE={rmse:.4f}, MeanDiff={mean_diff:.4f}")
    
    if rmse > tolerance_rmse:
        return f"{name}: RMSE {rmse:.4f} > tolerance {tolerance_rmse}"
        
    return None

def compare_data(golden, current, path=""):
    """Recursively compare JSON data with special handling for timeseries."""
    
    # Special Handling for Timeseries Keys
    if path.endswith("timeseries"):
        # We handle the dictionary of arrays specifically here
        ...
        for key in golden.keys():
            ...
            # 1. Ignore Setpoint/HVAC Output specific element deviations
            if key in ['setpoint', 'hvac_output_btu']:
                continue
                
            # 2. Statistical Comparison for Temperature
            if key in ['sim_temp', 'indoor_temp_pred', 'actual_temp']:
                err = compare_timeseries_stats(golden[key], current[key], f"{path}.{key}")
                if err: errors.append(err)
                continue
                
            # 3. Default List Comparison for others (e.g. timestamps)
            # Actually, just recurse for others?
            # But compare_data recurses into lists element-by-element which is slow and strict.
            # Let's trust strict compare for timestamps.
            err = compare_data(golden[key], current[key], f"{path}.{key}")
            if err: errors.append(err)
    ...
    if isinstance(golden, dict):
        ...
        # Tolerance check
        # Use tighter tolerance for Summary Energy, loose for others?
        # Default 0.1 is fine for energy which is ~20 kWh. 0.5% error.
...
def run_tests():
    ...
    print(f"Running {len(TESTS)} Regression Tests (Statistical)...")
    print("="*60)
